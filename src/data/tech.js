export const TECH = [
  // { name: "Node.js",      role: "Runtime",            icon: "⚡",  color: "#68a063" },
  { name: "Express.js",   role: "Web Framework",       icon: "🚂",  color: "#ffffff" },
  { name: "mysql2",       role: "DB Driver (promise)", icon: "🔌",  color: "#f59e0b" },
  // { name: "dotenv",       role: "Config / Env",        icon: "🔑",  color: "#8b5cf6" },
  // { name: "CORS",         role: "Cross-Origin",        icon: "🌐",  color: "#10b981" },
  { name: "React 18",     role: "Admin UI",            icon: "⚛️",  color: "#61dafb" },
  { name: "Tailwind CSS", role: "Styling",             icon: "🎨",  color: "#38bdf8" },
  { name: "Flutter",      role: "app freamwork",       icon: "📱",  color: "#38bdf8" },
  { name: "figma",      role: "Design",       icon: "🖌️",  color: "#38bdf8" },
];

export const ARCHITECTURE = [
  { label: "Client / Flutter App", color: "#f59e0b" },
  { label: "→",                    color: "#374151" },
  { label: "Express.js",           color: "#10b981" },
  { label: "→",                    color: "#374151" },
  { label: "Admin / Express js",   color: "#8b5cf6" },
];

export const ROUTERS = [
  { label: "Router: /products", color: "#f59e0b" },
  { label: "Router: /users",    color: "#8b5cf6" },
  { label: "Router: /posts",    color: "#10b981" },
];